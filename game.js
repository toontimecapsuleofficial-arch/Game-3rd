...
function collisionDrop(){
 ...
 G._hit=true;
 const _ovFrac=(_R2-_L2)/m.w;
 const _nx=_ovFrac<0.45?top.x:clamp(m.x,-20,W-m.w+20); /* close case — snap flush onto the block below */
 m._ov=_ovFrac;
 placeBlock(_nx,m.w,false,m,top.w);
 G._hit=null;
...
function dropBlock(){
 ...
 if(perfect){nx=top.x;G.combo++;}
  ...
  G.combo=0;
  endFever(false);
  if(m._ov<0.45){ /* close case — nearly slid off, so let it snap flush onto the block below */
   nx=top.x;
  }else{
